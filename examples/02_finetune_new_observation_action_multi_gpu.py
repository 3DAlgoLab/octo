...
"""Note: 
    This code generated by aider & sonnet.
    But too slow and memory errors occurred.  
"""
...
def main(_):
    ...
    def train_step(state, batch):
        ...
        def device_update(rng, state):
            ...
            grads = jax.tree_map(
                lambda x: x / (per_device_batch_size * FLAGS.grad_accum_steps), grad_acc
            )
            return new_rng, (losses, infos, grads)

        new_rngs, (losses, infos, grads) = jax.vmap(device_update)(
            rngs, jax.tree_map(lambda x: jnp.array([x] * n_devices), state)
        )
