...
def main(_):
    ...
    per_device_batch_size = FLAGS.batch_size // n_devices
    train_data_iter = (
        dataset.repeat()
        .unbatch()
    ...
    def loss_fn(params, batch, rng, train=True):
        rngs = {"dropout": rng}
        bound_module = model.module.bind({"params": params}, rngs=rngs)
    ...
    def train_step(state, batch):
        def loss_and_grad(params, dropout_rng):
        ...
        def inner_step(carry, _):
            rng, state, grad_acc = carry
            rng, dropout_rng = jax.random.split(rng)
            loss, (info, grads) = loss_and_grad(state.model.params, dropout_rng)
            grad_acc = jax.tree_map(lambda acc, g: acc + g, grad_acc, grads)
            return (rng, state, grad_acc), (loss, info)

        rng, _ = jax.random.split(state.rng)
        rngs = jax.random.split(rng, n_devices)
        
        def device_update(rng, state):
            grad_acc = jax.tree_map(jnp.zeros_like, state.model.params)
            (new_rng, _, grad_acc), (losses, infos) = jax.lax.scan(
                inner_step,
                (rng, state, grad_acc),
                None,
                length=per_device_batch_size * FLAGS.grad_accum_steps
            )
            grads = jax.tree_map(lambda x: x / (per_device_batch_size * FLAGS.grad_accum_steps), grad_acc)
            return new_rng, (losses, infos, grads)
        
        new_rngs, (losses, infos, grads) = jax.vmap(device_update)(rngs, jax.tree_map(lambda x: jnp.array([x] * n_devices), state))

        info = jax.tree_map(lambda x: jnp.mean(x, axis=(0, 1)), infos)
        grads = jax.tree_map(lambda x: jnp.mean(x, axis=0), grads)
        new_state = state.apply_gradients(grads=grads, rng=new_rngs[-1])
    ...
    for i in tqdm.tqdm(range(5000), total=5000, dynamic_ncols=True):
        ...
        # batch = jax.tree_map(lambda x: x.numpy(), batch)  # Convert to numpy arrays, already numpy
        sharded_batch = jax.tree_map(
            lambda x: x.reshape((n_devices, per_device_batch_size) + x.shape[1:]), batch
        )
        train_state, update_info = p_train_step(train_state, sharded_batch)

        if (i + 1) % 100 == 0:
            update_info = jax.device_get(update_info)
            update_info = jax.tree_map(
                lambda x: x[0], update_info
        ...
        if (i + 1) % 1000 == 0:
            # save checkpoint
            unreplicated_state = jax.device_get(unreplicate(train_state))
            save_dir = os.path.join(FLAGS.save_dir, f"checkpoint_{i}")
            os.makedirs(save_dir, exist_ok=True)
            unreplicated_state.model.save_pretrained(save_dir)
